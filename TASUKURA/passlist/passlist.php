<?php
    ...
    if ($_SESSION['is_logged_in']) {
        ...
        if ($search_word) {
            // バインドパラメータを使用したSQL
            $sql = "SELECT pass_id, URL, passName FROM PassList WHERE user_id = :user_id AND (URL LIKE :search_term OR passName LIKE :search_term)";
            $stmt = $pdo->prepare($sql);

            // バインドパラメータに値をバインド
            $stmt->bindValue(':user_id', $_SESSION['user_id'], PDO::PARAM_INT);
            $stmt->bindValue(':search_term', '%' . $search_word . '%', PDO::PARAM_STR);

            $stmt->execute();
            $list = $stmt->fetchAll(PDO::FETCH_ASSOC);
        } else {
            // 検索ワードが空の場合、全件を取得
            $sql = "SELECT pass_id, URL, passName FROM PassList WHERE user_id = :user_id";
            $stmt = $pdo->prepare($sql);
